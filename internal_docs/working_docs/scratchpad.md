...
- Low health maps to full visibility. Above the configured threshold, the curve eases toward the current fade base alpha to avoid a hard snap during regen ticks.
- Health events are debounced and must not interrupt an active fade. After the base fade has reached its target, one health event per combat/OOC cycle may force a fresh fade from visible alpha to soften threshold release.
