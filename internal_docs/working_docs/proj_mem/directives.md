...
- `internal_docs/tests_tools/` is long-term capture for probes, experiments, developing tests, and local diagnostic helpers. Do not delete files from it during cleanup unless the user explicitly asks to remove that specific artifact.
...
- Keep durable knowledge in `proj_mem/` or `internal_docs/completed_features/`; keep transient notes in `scratchpad.md`.
...
- Completed feature investigations belong in `internal_docs/completed_features/`.
