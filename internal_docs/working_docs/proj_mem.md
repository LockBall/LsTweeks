...
- Hot path performance: `M._event_cache` is a flat pre-baked table keyed by event name; each slot holds pre-resolved `path`, `soundkit_id`, `muted`, `sound_off`, `channel`. `handle_event` does one table lookup and walks a small array — no DB access, no `apply_defaults` at event time. Cache is rebuilt by `M.rebuild_event_cache()` which is called at the end of every `apply_sound_levels()`. `get_db()` applies defaults only once per session (guarded by `M._defaults_applied`); cleared on reset.
