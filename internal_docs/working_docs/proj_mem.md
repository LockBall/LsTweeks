...
- Skyriding Vigor Scale is style-specific. `M.set_db_value("scale", value)` routes to the active `style_layouts` entry and style switching resyncs the Scale slider. Active style scale/fill color and decor position helpers live in `sv_bar.lua` with the style definitions.
...
- Skyriding Vigor end-decoration placement uses per-style defaults in `DECOR_STYLES` in `sv_bar.lua` (`decor_node_gap_x`, `offset_y`, `scale`, `scale_x`, `scale_y`), with saved user X/Y/scale overrides under `db.decor_layouts`. X/Y no longer use shared `WING_LAYOUT` fallback values.
