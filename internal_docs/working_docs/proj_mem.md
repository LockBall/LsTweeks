...
- Retail 12.x health APIs can return Secret Values from tainted addon paths. Player Frame health fade is strictly OOC: combat cancels fade and sets plain alpha `1`. In-game testing showed `UnitHealth`, `UnitHealthPercent`, `CurveConstants.ScaleTo100`, custom `UnitHealthPercent` curves, and PlayerFrame health bars all return secret current-health values OOC. The usable pattern is pass-through only: compute a normal time-based base alpha, build a `C_CurveUtil` curve where low health maps to `1` and health above the threshold eases toward the base alpha, then pass the `UnitHealthPercent("player", true, curve)` result directly to `PlayerFrame:SetAlpha()`. Do not use Lua comparisons/arithmetic/string conversion on current health or curve output. Health events should not stop or restart an active fade; after the base fade is already at target, health events only refresh the gated target alpha and must not force a full-visible restart.
...
- Multi-file targets can declare `replacement_assets = { ... }`; `apply_replacement_paths()` builds `replacement_path_sets`, and playback cycles through the matching preset path for each asset. Fishing Bobber uses this model for its three original FileDataIDs and three 20-file replacement folders. Keep Fishing Bobber defaulted to Original until its live runtime trigger is confirmed.
- Fishing Bobber bite timing is not currently exposed through tested Lua hooks/APIs (sound hooks, soft-interact/world-loot/object state, tooltip APIs, vignettes, channel updates, or gamepad vibration hooks). The implemented user-facing control is **Fishing Focus**: an opt-in second channel-volume profile that caches current Sound_* CVars on Fishing channel start (`131476`), applies configured Master/SFX/Music/Ambience/Dialog values, and restores cached values on channel stop/reset/logout.
...
- Sound APIs: `PlaySoundFile(fileDataID_or_path, channel?)` returns `(willPlay, soundHandle)`. `C_Sound.PlaySound(soundKitID, uiSoundSubType?)` returns `(success, soundHandle)`. `MuteSoundFile` / `UnmuteSoundFile` accept `number|string`. `StopSound(handle, fadeoutTime?)`. The global `PlaySound` is an alias for `C_Sound.PlaySound` (set in `Blizzard_SharedXML/Sound.lua`). `C_Sound.MuteSoundFile` and `C_Sound.UnmuteSoundFile` do not appear in Ketho’s `SoundDocumentation.lua` — they exist as globals only. Always resolve sound API upvalues at file load; do not re-check `C_Sound` on every call.
