...
modules/
  ...
  skyriding_vigor/       restored Skyriding Vigor display using Blizzard atlas assets and spell charges
    ...
    sv_bar.lua           Skyriding Vigor bar visuals, style definitions/helpers, Blizzard atlas slots, wing layout, positioning, bar setting specs
    sv_fade.lua          Skyriding Vigor alpha transitions and full-charge fade policy
    sv_state.lua         Skyriding Vigor charge, glide, flying, and advanced-flight mount state detection
    sv_gui.lua           Skyriding Vigor settings UI and control synchronization
    sv_main.lua          Skyriding Vigor runtime conductor, DB normalization, events/ticker, category bootstrap
...
- Skyriding Vigor Scale and Node Color are style-specific. `M.set_db_value("scale", value)` and `M.set_db_value("node_color", value)` route to the active `style_layouts` entry, and style switching resyncs those controls. Active style scale/node-color/fill-color and decor position helpers live in `sv_bar.lua` with the style definitions.
- Skyriding Vigor Fill Color is style-specific and tints the active fill/full-fill status bar texture via `SetVertexColor`. Non-white fill colors automatically desaturate the fill texture first. A Fill Brightness multiplier was tested and discarded because RGB multiplication plus channel clamping shifted selected hues unpredictably.
...
- Skyriding Vigor end-decoration placement uses per-style defaults in `DECOR_STYLES` in `sv_bar.lua` (`decor_node_gap_x`, `offset_y`, `scale`, `scale_x`, `scale_y`, `decor_color`), with saved user X/Y/scale/color overrides under `db.decor_layouts`. X/Y no longer use shared `WING_LAYOUT` fallback values.
