...
- Avoid using `Set-Content` or command-output capture for source rewrites unless the newline behavior is explicitly tested. In this repo, PowerShell command-output arrays, implicit stringification, and careless joins have caused Lua files to collapse into one physical line during generated rewrites.
...
- Region validation and source outlines live in `internal_dev/tests_tools/check_regions.ps1`.
...
- Treat the live outline as the source-file TOC. Do not add manual Lua TOCs unless a generated/validated TOC workflow is introduced.
