...
- Centralized tooltip handling lives in `functions/tooltip.lua`. General addon-authored help/line content uses `addon.CreateOwnedTooltip()`, `addon.GetOwnedTooltip()`, `addon.ShowOwnedTooltipLines()`, `addon.ShowOwnedTooltip()`, `addon.HideOwnedTooltip()`, `addon.AttachTooltip()`, and `addon.AttachTooltipToTargets()`; its `TooltipBackdropTemplate` is a plain Frame plus native nine-slice art, with no tooltip logic. Secret Aura/spell content is the narrow exception: use `addon.ShowNativeAuraTooltip()` / `addon.ShowNativeSpellTooltip()` and `addon.HideNativeTooltip()`, never call global `GameTooltip` directly from a module.
...
- Tooltip APIs: route every tooltip through `functions/tooltip.lua`. Use the addon-owned plain renderer for addon-authored or guarded `C_TooltipInfo` line data; it owns native fonts, nine-slice skin, content sizing, quadrant anchoring, and screen clamping. Restricted Aura descriptions cannot be reconstructed safely from secret values, so Aura Frames must pass the instance ID directly to the shared global `GameTooltip:SetUnitAuraByAuraInstanceID` delegate (or readable spell ID to `SetSpellByID`) without inspecting the rendered result. Do not create an addon-owned `GameTooltip`, do not wrap its setters in `securecallfunction`, and do not read/measure its lines: that older combination retained LsTweeks taint and later broke Blizzard map POI widget arithmetic. Direct shared-tooltip delegation was validated in game on 2026-07-19 with rich short-buff/debuff tooltips in and out of combat followed by delve/map POI hover without recurrence.
