...
- Objective Tracker APIs: `ObjectiveTrackerFrame`, `CampaignQuestObjectiveTracker`, `QuestObjectiveTracker`, and `AchievementObjectiveTracker` expose `SetCollapsed`/`IsCollapsed`. Never collapse the parent tracker. Current section Auto-Collapse defers native `SetCollapsed` through the source-supported `securecall` candidate and preserves manual-open state; the full risk boundary and live-validation status belong in `modules/objectives.md`.
