...
- The addon-owned tooltip factory lives in `functions/tooltip.lua`: `addon.CreateOwnedTooltip()`, `addon.GetOwnedTooltip()`, `addon.ShowOwnedTooltipLines()`, `addon.ShowOwnedTooltip()`, `addon.HideOwnedTooltip()`, `addon.AttachTooltip()`, `addon.AttachTooltipToTargets()`. The frame uses `TooltipBackdropTemplate` (plain Frame plus `NineSlicePanelTemplate` child with native tooltip art/colors, no tooltip logic) and auto-sizes width to its widest line. Use it instead of direct module use of global `GameTooltip`.
...
- Tooltip APIs: route addon help and rich content through the centralized addon-owned plain tooltip renderer (`functions/tooltip.lua`), which owns native tooltip fonts, the `TooltipBackdropTemplate` nine-slice skin, content-sized width, quadrant-aware owner anchoring, and screen clamping. Copy only guarded `C_TooltipInfo` text/color data into its line model; never bind addon data to a live `GameTooltip`, because isolated frames and `securecallfunction` can still taint later Blizzard secret-value widget layout.
