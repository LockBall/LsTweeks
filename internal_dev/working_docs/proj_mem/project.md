...
- Tooltip APIs: route every tooltip through `functions/tooltip.lua`. Use the addon-owned plain renderer for addon-authored or guarded `C_TooltipInfo` line data; it owns native fonts, nine-slice skin, content sizing, quadrant anchoring, and screen clamping. Restricted Aura descriptions cannot be reconstructed safely from secret values, so the factory passes Aura instance or readable spell IDs to direct native setters on one dedicated LsTweeks `GameTooltip`. Never put Aura data or ownership onto Blizzard's shared global `GameTooltip`, inspect/measure native rendered lines, or wrap native setters in `securecallfunction`. A shared-global delegate appeared clean in short testing but produced delayed Area POI widget-layout taint on 2026-07-20; isolation is therefore a required regression contract, not an implementation detail.
