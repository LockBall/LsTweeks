...
- `addon.CreateBackgroundRegion()` in `functions/ui_helpers.lua` is the reusable texture-backed runtime background controller. It follows its parent through anchors without reading dimensions and caches insets, RGBA, and shown state; modules retain ownership of color/visibility policy and parent-specific combat restrictions.
