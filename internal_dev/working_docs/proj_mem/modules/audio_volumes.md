...
- `av_profiles.lua` owns the Audio Volumes snapshot schema and applies loaded settings through `on_reset_complete`; the shared profile manager owns storage and standard Profiles-tab controls. Profiles include sound targets and situation settings but exclude UI selection/session state. ARM reset keeps profiles by default through **Keep Profiles**.
