...
- `style_layouts.<style>.scale`: per-node-style scale override used by the Scale slider.
...
- `sv_defaults.lua` intentionally seeds `style_layouts` sparsely. Treat missing per-style fields as normal; use `sv_styles.lua` layout helpers/normalization to initialize them from the current style defaults.
...
- Register `UNIT_ENTERED_VEHICLE` and `UNIT_EXITED_VEHICLE` through `RegisterUnitEvent(..., "player")`; other unit vehicle changes must not refresh the vigor display.
...
- Build the vigor frame tree eagerly at module startup. Reopen lazy construction only with measured startup cost, since eager construction keeps first mount, Move Mode, and Fill Test presentation immediate.
...
- `sv_bar.lua` caches `M.get_render_context(db)` for style-dependent slot rendering. It carries the active DB, `style_key`, `style`, `frame_atlas`, and `spark_atlas`; `M.invalidate_layout()` clears it whenever layout/style changes can affect those values. `M.set_slot_state()` stamps each slot with the resolved render values, and `M.update_filling_slot_progress()` reuses those slot-local values so active progress ticks do not rebuild style/atlas context.
...
- Race-detection events refresh the module only when Bronze Timepiece presence changes the active race state; unchanged event bursts must return before frame/layout/charge work.
