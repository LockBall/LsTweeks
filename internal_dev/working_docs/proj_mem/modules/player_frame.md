...
- `health_release_speed` setting changes do not need the threshold-change cache-clear path; the curve cache signature includes release speed, so the next fade apply/health refresh self-invalidates the cached curve.
