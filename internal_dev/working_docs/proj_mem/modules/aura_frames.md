...
- `af_managed.lua` is the thin WoW 12.1 managed-Aura lifecycle boundary: availability, container creation, initialization tracking, safe accessibility checks, and module lifecycle gating only. `AuraContainer:SetEnabled` controls processing separately from visibility, so every lifecycle transition must keep both states synchronized. Capability code calls Blizzard AuraContainer APIs visibly; do not grow this into a generic renderer or hidden dispatcher.
- `af_managed_presets.lua` owns the shared native presentation for Combined Buffs and Debuffs. Combined Buffs is a distinct `combined` preset with its own saved settings and position and an unrestricted `HELPFUL` filter; Long remains a separate existing preset for later managed separation work. Debuffs uses `HARMFUL`. Neither managed frame owns legacy Aura-update events, indexed scans, an icon pool, or addon Aura-data reads. Each backend preinitializes one bar group and one icon group because `initializeFrame` bindings are immutable; only the selected group has a nonzero maximum. OOC Bar Mode changes park the old group, activate the other, and reapply its layout without recreating the backend. Bar buttons are 18px rows with native icon, name, duration, stack, and fill bindings; icon cells contain a 32px native icon, duration row below it, and bottom-right stack binding on the icon. Managed duration FontStrings inherit a category-owned addon Font object, allowing OOC Timer Text, face, size, bold, and color changes without mutating constrained AuraButtons or Blizzard-owned duration values; disabling Timer Text also removes the icon timer row from flow layout.
- Managed Debuff AuraButtons use their native tooltip path with mouse motion enabled and combat tooltips hidden. Do not add addon `OnEnter`/`OnLeave` handlers or reconstruct Aura tooltip data; native hover is also the safe diagnostic for identifying which Aura Blizzard bound to an unexpected icon.
...
- Managed frames intentionally own no `PLAYER_ENTERING_WORLD` or `UNIT_AURA` handlers, so runtime startup must explicitly refresh their addon-owned shell visibility after enabling the managed engine. Their parent shells retain only `PLAYER_REGEN_DISABLED/ENABLED` to apply saved out-of-combat fading and restore full combat alpha; pass the event's explicit combat state into the fade helper instead of immediately re-querying `InCombatLockdown()`. These presentation events must not invoke Aura scans. Do not assume native container activation also shows its addon parent.
- `/lst status aura_frames` includes one `managed=...` record per managed backend with unit, feature, engine, container, group, created-AuraButton, and accessible-AuraButton counts. Never inspect `AuraButton:IsShown()` and never attach `OnShow`/`OnHide` scripts: Blizzard drives `AuraButton:SetShown()` with a secret boolean, and any addon script handler makes that native write illegal.
...
- Managed Buffs and Debuffs route through `update_managed_preset_frame()` before the legacy `self.icons` gate. Their containers own Aura updates; addon-owned shells own enabled/move-control visibility and out-of-combat alpha, including combat-state transitions.
...
- Aura Frames owns the default-off `shared_background_color_enabled`; independent shared Frame BG, Bar BG, Buff Bar, Debuff Bar, Bar Text, and Timer Text colors; and default-on `sync_bar_bg*`/`sync_bar_color*`/`sync_text_color*` selections. One BG Colors participation selection applies both background colors, one Bar Colors selection applies the type-resolved Buff/Debuff fill, and one Text Colors selection applies both text colors. Each resolves locally before the independent whole-module Shared Colors override. The tab's Disable OOC Fade checkbox is a linked view of Shared Colors' single global fade policy, not Aura-owned profile state.
- Aura Frames also registers `supports_ooc_fade = true`; each frame's saved Fade OOC value passes through Shared Colors so global `Disable OOC Fade` can temporarily restore full alpha without changing frame settings.
...
- `af_icon_layout.lua` preallocates one frame-background texture per display count during its out-of-combat layout pass. In combat, `M.update_combat_background()` switches only the matching texture's visibility while the shell backdrop stays transparent; do not resize, reanchor, or create these variants in combat.
...
- `af_test_aura.lua` owns the configurable long-preview ranges, phase order, handoff/zero holds, and optional per-range `seconds_per_unit` pacing. Runtime builds the phase list and exposes its derived starts/cycle time to tests; tests must not duplicate phase order or duration arithmetic. Restored saved previews initialize paused so the button offers Play after reload, while manually enabling a preview starts it. The global control temporarily enables previews on every enabled frame without replacing the individual controls in the Frames tabs. The shared cycling long preview must reclassify both Long→Short at the threshold and Short→Long after its restart; real auras only move Long→Short. Tooltip fallbacks display full days/hours/minutes/seconds.
...
- Outside combat, each unified scan prewarms every safely identified entry in its bounded shared Aura map, including entries beyond visible icons; existing entries are lookup-only and unavailable hidden entries are probed at most twice. Visible icons retain their two-retry prewarm for late-loading data and the pre-combat attempt on `PLAYER_REGEN_DISABLED`. Cache both Aura-instance and reusable spell aliases so a spell cache can seed a newly seen instance. The cache is runtime-only: clear Aura-instance keys and failed-attempt counters on `PLAYER_ENTERING_WORLD`, retain reusable spell keys, and clear everything on reload/logout.
...
- Aura frames own one shell-parented `frame.move_handle`, shown as a thin border only during Move Mode. Four 8px inward hit areas share the drag path and show the frame title through the owned tooltip; the center remains free for aura interaction. The bottom and right mover hit areas stop before the bottom-right 16px square, reserving that corner exclusively for the explicitly mouse-enabled resize grip. The border does not reserve layout space or depend on growth direction. It must remain owned and anchored only by the addon shell: anchoring an addon frame to AuraContainer makes it inherit `UntrustedLayoutScriptExecution` and WoW rejects `SetPoint()` even out of combat.
...
- Profiles and reset include Aura-owned shared frame/bar backgrounds, Buff/Debuff bar-fill colors, and every built-in/custom participation selection.
...
- The Shared BG Colors tab has **Frame Name**, **BG Colors**, **Bar Colors**, and **Text Colors** columns. Picker rows are Frame BG/Buff Bar/Bar Text, then Bar BG/Debuff Bar/Timer Text; `COLOR_PICKER_ROW_1_Y` and `COLOR_PICKER_ROW_2_Y` are the vertical layout knobs. Each frame has one participation checkbox per color column.
- Disable OOC Fade and the global test-aura control remain active when shared colors are disabled or globally overridden; individual test-aura controls remain in the Frames tabs. Shared pickers and participation controls require **Enable** and are inactive under the whole-module global override. BG Colors participation also makes the frame background visible. Matrix slots refresh after custom-frame and profile/reset changes without rebuilding frames.
