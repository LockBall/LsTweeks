...
## Aura Tooltips
- Product requirement: Aura tooltips must retain useful live combat details, including the effect description; name/duration-only or pre-combat-cache-only output is not an acceptable final solution. Cached data may enrich or bridge a live tooltip but must not replace live combat content. Error suppression is acceptable only if the rendering path remains functional and the underlying failure cannot wedge or disable later tooltips; merely hiding Lua error reports does not satisfy the requirement.
- Renderer architecture, secret-data rules, taint evidence, and shared regression contracts are owned in `../functions/tooltip.md`. Aura hover order is exact native Aura when explicitly non-secret, opaque live Aura with a known palette, explicitly non-secret native Aura spell, opaque live Aura with neutral colors, then guarded cached/basic lines.
- Guarded cached `C_TooltipInfo` lines rendered by the addon-owned plain tooltip remain the fallback when native/opaque rendering or identity is unavailable; guarded name/duration is final. Outside combat, each unified scan prewarms every safely identified entry in its bounded shared Aura map, including entries beyond visible icons; existing entries are lookup-only and unavailable hidden entries are probed at most twice. Visible icons retain their two-retry prewarm for late-loading data and the pre-combat attempt on `PLAYER_REGEN_DISABLED`. Cache both Aura-instance and reusable spell aliases so a spell cache can seed a newly seen instance. The cache is runtime-only: clear Aura-instance keys and their failed-attempt counters on `PLAYER_ENTERING_WORLD`, retain reusable spell keys, and clear everything on reload/logout.
...
- Aura-specific in-game acceptance: after a fresh reload, hover active buffs/debuffs and short-lived combat effects; they must retain useful live descriptions, not merely name and duration. The shared cross-surface taint regression checklist lives in `../functions/tooltip.md` `## Validation`.
...
- The Shared BG Colors tab uses a flat left-aligned 700px grid with no title or outer frame: Enable in column one, the globally linked Disable OOC Fade in column two, and the globally linked Enable Test Auras plus Play/Pause control in column three. The shared `CreateTestAuraControl()` factory also owns the matching All the Colors control. After the divider, a muted gray title bar spans **Frame Name**/**BG Colors**/**Bar Colors**/**Text Colors** columns. Its two picker rows are Frame BG/Buff Bar/Bar Text, then Bar BG/Debuff Bar/Timer Text. `COLOR_PICKER_ROW_1_Y` and `COLOR_PICKER_ROW_2_Y` set their top edges with standard control spacing. Each participation row has one checkbox for each color column. Only this matrix omits per-frame test-aura controls; the individual controls remain in the Frames tabs. Disable OOC Fade and the global test-aura control remain active when Shared BG Colors is disabled or globally overridden. When **Enable** is off, shared pickers and participation controls are inactive. The entire module-specific shared-color set is also inactive while Shared Colors has both Enable Global Color and Buffs & Debuffs participation checked. Selecting BG Colors also makes that frame background visible even if its local Frame BG setting is off. Slots refresh on custom create, rename, delete, profile load, and reset without rebuilding frames.
