...
- Timer text enable/format behavior is centralized in `af_functions.lua` via `M.get_timer_behavior()` and `M.is_timer_text_enabled()`. `af_render.lua` keeps every time label compact: up to two-decimal days below 10d and whole days at 10d+, decimal hours at 10h+, `9h59m` below 10h (or `9h` exactly), decimal minutes at 10m+ (with `10m`, not `10.0m`), `9m59s` below 10m (or `9m` exactly), and decimal seconds below one minute. The shared decimal-label formatter suppresses an all-zero fractional part for every unit (`1d`, `10h`, `10m`, `10s`). `af_icon_layout.lua` derives timer-slot width from the selected font, keeping the default icon footprint compact while accommodating larger configured fonts. Timer alignment remains layout behavior there.
- The Long frame test aura starts at whole days whenever enabled, then loops through boundary-focused non-linear countdown phases. Each phase begins just above a display cutoff and crosses it after about five seconds: 10d, 24h, 10h, 1h, 10m, and 1m. The final seconds phase accelerates from one minute to 10s, then counts the final 10 seconds down in real time and leaves 0s visible for one second before the preview loops. The Pause/Play button beside Test Aura freezes/resumes each frame's preview clock without changing saved settings. Tooltip fallbacks display full days/hours/minutes/seconds.
...
## Aura Tooltips
- Aura icons use their own addon-owned `GameTooltip`, separate from the plain tooltip used for settings help. Out of combat, bind live Aura data through `securecallfunction`-wrapped `SetUnitAuraByAuraInstanceID()` / `SetSpellByID()` calls for complete Blizzard rendering; in combat or if rich rendering fails, use cached `C_TooltipInfo` lines, then guarded name/duration text. Visible icons prewarm safe text lines out of combat, retry missed reads twice, and make one pre-combat attempt on `PLAYER_REGEN_DISABLED`. The cache is runtime-only: clear aura-instance keys on `PLAYER_ENTERING_WORLD`, retain reusable spell keys, and clear all keys on reload/logout.
