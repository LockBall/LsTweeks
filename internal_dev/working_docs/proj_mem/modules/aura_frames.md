...
- Renderer architecture, secret-data rules, taint evidence, and shared regression contracts are owned in `../functions/tooltip.md`. Aura hover always uses opaque live Aura first, then guarded cached/basic lines. Native Aura/spell renderers are disabled because predicate-approved OOC data can still carry secret line colors.
