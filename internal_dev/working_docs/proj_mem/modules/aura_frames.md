...
- Aura frames own one shell-parented `frame.move_handle`, shown as a thin border only during Move Mode. Four 8px inward hit areas share the drag path and show the frame title through the owned tooltip; the center remains free for aura interaction, and the resize grip overlays the bottom-right border corner. The border does not reserve layout space or depend on growth direction. It must remain owned and anchored only by the addon shell: anchoring an addon frame to AuraContainer makes it inherit `UntrustedLayoutScriptExecution` and WoW rejects `SetPoint()` even out of combat.
