...
- `af_managed_debuff.lua` owns AF12-05. Its first icon-only slice uses one managed `HARMFUL` group and no legacy Aura events, indexed scans, icon pool, or addon Aura-data reads; extend presentation there capability-by-capability.
...
- Managed frames intentionally own no `PLAYER_ENTERING_WORLD` or `UNIT_AURA` handlers, so runtime startup must explicitly refresh their addon-owned shell visibility after enabling the managed engine. Do not assume native container activation also shows its addon parent.
...
- Managed Debuffs route through `update_managed_debuff_frame()` before the legacy `self.icons` gate. Their container owns Aura updates; the addon-owned shell currently owns only enabled and move-control visibility.
