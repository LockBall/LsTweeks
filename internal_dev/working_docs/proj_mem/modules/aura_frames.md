...
- Real Aura icons use centralized isolated renderers. One lightweight LsTweeks `GameTooltip` combines `SharedTooltipArtTemplate` with `GameTooltipDataMixin` while omitting full `GameTooltipTemplate` widget ownership and `GameTooltip_OnHide`. Before `SetUnitAuraByAuraInstanceID`, require `C_Secrets.ShouldUnitAuraInstanceBeSecret` to return explicit `false`. Secret or indeterminate Auras with cached out-of-combat lines first use a separate data-mixin-free `SharedTooltipArtTemplate`: live `C_TooltipInfo` left/right text passes directly into native lines while safe cached colors are reapplied by line position, preserving known presentation without freezing text. Without a known palette, prefer `SetSpellByID` for readable spell identities, then opaque live Aura text with neutral colors. Never inspect secret text, color tables, wrap flags, or rendered measurements. Guarded cached lines remain next. Call allowed setters directly under `pcall`; never use `securecallfunction`. Track each successful renderer owner so Aura leave hides only its own tooltip. Tests must prove the shared global is untouched, the native tooltip cannot enter `GameTooltip_OnHide`, secret Auras never reach the native Aura processor, readable spell IDs reach the spell renderer, and opaque secret Auras retain live title/description plus known safe colors when available.
- Taint incident history: 2026-06-28 Area POI `UIWidgetTemplateTextWithStateMixin:Setup()` secret `textHeight`; 2026-07-01 and 2026-07-03 world quest `EmbeddedItemTooltip_UpdateSize()` secret width; 2026-07-03 Area POI `UIWidgetTemplateStatusBarMixin:InitPartitions()` secret `barWidth`; 2026-07-19 map POI `Setup()` secret `textHeight` with an isolated tooltip plus `securecallfunction` and rendered-line inspection; 2026-07-20 Area POI `LayoutFrame.lua` secret comparison after direct shared-global Aura delegation; 2026-07-21 repeated `GameTooltip_ClearWidgetSet()` failures with a dedicated full template, followed by a direct lightweight-renderer failure indexing a secret line-color table. The latter left `processingInfo` active and stopped later tooltips, establishing the pre-setter secret gate.
...
- Test after a fresh reload by hovering addon aura icons/bars for active player buffs and debuffs both out of combat and in combat; they must retain useful Blizzard descriptions, not merely name and duration. Then hover world quests, delve entrances, Quick Join, chat links, Bartender buttons, and other widget-bearing tooltips. Blizzard tooltip layout must not report secret-number arithmetic tainted by LsTweeks.
