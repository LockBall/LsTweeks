...
- `af_managed_debuff.lua` owns AF12-05. It uses one managed `HARMFUL` group and no legacy Aura-update events, indexed scans, icon pool, or addon Aura-data reads. Saved bar mode builds each AuraButton as an 18px row during `initializeFrame` and hands its icon, spell name, duration text, application count, and duration fill to native bindings; saved icon mode retains a 32px native icon cell.
...
- Every newly constructed Aura frame shell, title bar, bottom title bar, and resizer starts hidden; only `update_auras()` may show them from saved enabled/move state. Disabled startup events intentionally return early, so relying on CreateFrame's default shown state exposes a white move shell after reload.
...
- Managed Debuffs route through `update_managed_debuff_frame()` before the legacy `self.icons` gate. Their container owns Aura updates; the addon-owned shell owns enabled/move-control visibility and out-of-combat alpha, including combat-state transitions.
