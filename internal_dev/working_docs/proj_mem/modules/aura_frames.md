...
- Aura icon and settings-help tooltips must go through `functions/tooltip.lua`. Settings help uses the addon-owned plain renderer; real Aura icons use its narrow shared-native delegates. Never recreate the removed addon-owned `GameTooltip` plus `securecallfunction` path. The complete rule lives in `## Aura Tooltips` and project.md Tooltip APIs. Taint incident log: 2026-06-28 Area POI `UIWidgetTemplateTextWithStateMixin:Setup()` secret `textHeight`; 2026-07-01 and 2026-07-03 world quest `EmbeddedItemTooltip_UpdateSize()` secret width; 2026-07-03 Area POI `UIWidgetTemplateStatusBarMixin:InitPartitions()` secret `barWidth`; 2026-07-19 map POI `Setup()` secret `textHeight` recurred with an isolated addon-created `GameTooltip` whose setters were wrapped in `securecallfunction`.
...
## Aura Tooltips
- Real Aura icons first use the centralized direct native delegate: shared global `GameTooltip:SetUnitAuraByAuraInstanceID`, or `SetSpellByID` for a readable spell-only identity. This is the only path that preserves Blizzard's full rich rendering for restricted/secret short buffs and debuffs. The delegate must set the owner, call the shared tooltip method directly under `pcall`, and show/hide it without reading `NumLines`, text, width, height, or other rendered state. Track the successful native owner; an Aura leave may hide only when its caller, stored owner, and `GameTooltip:GetOwner()` match, and must clear stale local ownership without hiding a replacement tooltip. Do not create an addon-owned `GameTooltip`, use `GameTooltipTemplate`, or add `securecallfunction`; that previously tainted later map POI widget layout. Confirmed in game 2026-07-19: rich short buffs/debuffs worked in and out of combat, then a delve entrance/map POI tooltip rendered without the former Lua error.
