...
- Aura Bar Mode permits only DOWN/UP growth. Entering Bar Mode converts a saved LEFT/RIGHT selection to DOWN and refreshes the shared dropdown to its vertical-only option set; runtime normalization independently enforces DOWN for horizontal bar values restored through saved data or profiles. Icon mode exposes all four directions.
