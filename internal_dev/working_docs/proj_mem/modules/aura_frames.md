...
- Aura icon and settings help tooltips must use the addon-owned tooltip factory in `functions/tooltip.lua`, never any `GameTooltip`; the complete rule lives in `## Aura Tooltips` and project.md Tooltip APIs. Taint incident log motivating it: 2026-06-28 Area POI `UIWidgetTemplateTextWithStateMixin:Setup()` secret `textHeight`; 2026-07-01 and 2026-07-03 world quest `EmbeddedItemTooltip_UpdateSize()` secret width; 2026-07-03 Area POI `UIWidgetTemplateStatusBarMixin:InitPartitions()` secret `barWidth`; 2026-07-19 map POI `Setup()` secret `textHeight` recurred even with `securecallfunction`-wrapped rich renderers, which is why that approach was removed.
...
## Aura Tooltips
- Aura icons use the centralized addon-owned plain tooltip renderer. Rich content comes from guarded cached `C_TooltipInfo` left/right text, colors, and wrapping; only guarded name/duration text is used when cached data is unavailable. Never bind live Aura data to a `GameTooltip`: even an isolated frame and `securecallfunction` can retain LsTweeks taint until a later Blizzard map POI widget performs layout arithmetic on secret values. Visible icons prewarm safe text lines out of combat, retry missed reads twice, and make one pre-combat attempt on `PLAYER_REGEN_DISABLED`. The cache is runtime-only: clear aura-instance keys on `PLAYER_ENTERING_WORLD`, retain reusable spell keys, and clear all keys on reload/logout.
