...
## Aura Tooltips
- Aura icon tooltips attempt rich addon-owned tooltip rendering out of combat via `SetUnitAuraByAuraInstanceID()` / `SetSpellByID()` when IDs are not secret values. Visible rendered icons also prewarm safe text lines from `C_TooltipInfo.GetUnitAuraByAuraInstanceID()` / `GetSpellByID()` while out of combat, so users should not need to hover every aura before combat. In combat, or after a failed rich call, render cached full text lines; if no cache exists, fall back to name and duration only.
