...
- The General tab has BG Colors, Bar Colors, and Text Colors columns. BG Colors stacks Frame BG and Bar BG; Frame BG affects Aura frame backgrounds only, while Bar BG independently affects their bar backgrounds. Bar Colors stacks Buff Bar and Debuff Bar; Text Colors stacks Bar Text and Timer Text. The Buff/Debuff fill colors default to the matching Aura Frames Bar Color defaults. These overrides apply only while All the Colors and Buffs & Debuffs participation are enabled; local Aura colors remain saved underneath.
