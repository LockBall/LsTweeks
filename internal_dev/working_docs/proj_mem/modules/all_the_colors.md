...
- `atc_defaults.lua` owns global policy defaults, registry state, and the ordered global preset palette.
...
- `atc_logic.lua` owns `M.resolve_color(module_key, target_key, local_color)` with precedence `global -> caller-owned input` and `M.resolve_module_color(module_key, color_key, local_color)` for registered module-wide style overrides.
...
- The General tab has Frame BG and Bar Colors columns. Frame BG owns the preset/custom global background color; Bar Colors owns the Aura Frames Bar Mode overrides for Bar Color, Bar Text Color, and Bar BG Color. All four apply only while All the Colors and Buffs & Debuffs participation are enabled; local Aura colors remain saved underneath.
