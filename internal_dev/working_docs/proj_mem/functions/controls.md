...
- `font_catalog.lua`: addon-wide selectable-font definitions, bold faces, semantic Blizzard-default roles, font application, previews, and the popup's low-level font dropdown.
- `font_picker.lua`: `CreateFontPicker()` launchers plus one lazily created transactional Text Options popup shared by all consumers.
- `ui_helpers.lua`: `CreatePopupFrame()` owns default popup chrome. It uses the tiled Blizzard Marble background by default and accepts `background_texture` or `background_atlas`, tiling, tint, border, inset, strata, level, and clamping overrides.
...
- `CreateColorPicker()` owns the single live system-picker session and its neutral default preset palette; callers may replace both preset order and values through options. Clear prior swatch/opacity callbacks when a session closes, cancels, hides, or is replaced so a later picker cannot write the previous control. Snapshot native popup sizing/anchors before applying the custom layout, then restore them and hide injected controls on every close path. Reset/cancel callbacks apply immediately; live previews are coalesced.
...
- Configurable-font consumers use `addon.CreateFontPicker()` for the launcher/transaction and `addon.ApplySelectedFont()` at runtime; `CreateFontDropdown()` remains the popup's low-level family selector. Callers supply dynamic Color, Font, Size, Bold, and Outline bindings, defaults, semantic role, and preview callback. Edits preview live; Save retains them, while Cancel, dismissal, disable, or launcher replacement restores the opening snapshot and Reset previews caller defaults. Font definitions and role-aware Game Defaults stay in `font_catalog.lua`; bold eligibility preserves the saved preference, and font sizes retain fractional values. Keep this factory font-specific until a second dense popup consumer proves reusable seams; then extract only launcher state, popup chrome/placement, exclusive session lifecycle, snapshot/restore, and terminal-button behavior while leaving field layout, bindings, eligibility, validation, and preview policy with the domain owner.
...
## Validation
- `test_control_factories.lua` owns silent-setter recovery, play/pause state, slider throttling, callback-free timer regressions, and shared growth-direction/font factory coverage. Extend it when changing shared callback/container behavior.
