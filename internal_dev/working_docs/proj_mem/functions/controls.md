...
- Configurable-font consumers use `addon.CreateFontPicker()` and `addon.ApplySelectedFont()`; `CreateFontDropdown()` is the popup's low-level family selector. Each launcher supplies dynamic Color, Font, Size, Bold, and Outline bindings plus one caller-owned preview callback; optional `column_gap` controls the horizontal gap between the popup columns and defaults to 20 pixels. Every edit writes through for live preview; the size slider uses the shared immediate/throttled slider contract. The singleton popup snapshots on open; Cancel, hide, disable, or launcher replacement restores the snapshot, Save retains it, and Reset previews caller-provided defaults within the transaction. Launcher and popup-dropdown labels explicitly apply the selected color after installing their private preview FontObjects so role-specific Blizzard defaults cannot leak native gold/white colors. Font paths, labels, bold faces, and role-aware (`body`, `timer`, `stack`) Game Defaults stay in `font_catalog.lua`; selectable faces are Source Code Pro plus Blizzard's Friz Quadrata, Arial Narrow, Morpheus, and Skurri. Bold eligibility never discards the saved preference, and fractional sizes remain intact.
