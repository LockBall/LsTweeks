...
- Archive a processed error inbox: `pwsh.exe -NoProfile -ExecutionPolicy Bypass -File internal_dev/tests_tools/archive_lua_error_batch.ps1 -Path internal_dev/working_docs/ToDo/new_issue.txt -Label <short-label> -ClearInbox`; preserves raw export and `condensed.md` under `working_docs/reference/error_batches/unreviewed/` before resetting the inbox. Classify reviewed batches through that archive's `README.md`.
