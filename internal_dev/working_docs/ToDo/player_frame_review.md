...
1. [x] Redundant full re-fade ticker when already at target. Every `M.update_player_frame()` while `STATE_FADED` — each PlayerFrame OnShow via the hook at `pf_fade.lua:273-278`, plus every settings change — runs `apply -> queue_apply -> begin_fade` with `start_alpha == target`, spinning a 0.1s ticker for up to `fade_length` seconds doing no-op `SetAlpha` plus `UnitHealthPercent` pcalls each tick. Early-exit `begin_fade` when `math_abs(target - start_alpha) <= ALPHA_EPSILON`: apply once, set `STATE_FADED`, skip the ticker. Resolved 2026-07-07: `begin_fade()` now applies the target once and exits when start and target alpha already match, with a headless regression covering refresh while faded.

2. [x] Health-event timer churn with nothing to do. `sync_fade_events` registers `UNIT_HEALTH`/`UNIT_MAXHEALTH` whenever fade is on (`pf_main.lua:88-89`), and `queue_health_update` cancels and allocates a fresh `C_Timer.NewTimer` per event (`pf_fade.lua:309-313`) even though `on_health_update` only acts in `STATE_FADED` (`pf_fade.lua:252-256`) and the gate no-ops entirely at `threshold <= 0` (`pf_fade.lua:86`). Early-exit `queue_health_update` unless `state == STATE_FADED` (a fade completing inside the 0.1s debounce is covered by the next health event), and optionally skip registering health events when the threshold is 0. Resolved 2026-07-07: queued health debounce now only starts in `STATE_FADED` with a positive health threshold, with a headless regression covering delay, fading, faded, and threshold-off states.

3. [deferred: profiling-gated] In-combat `UNIT_HEALTH` fan-out is pure overhead. Player `UNIT_HEALTH` fires constantly in combat; each event walks loader OnEvent -> `handle_runtime_event` -> `queue_health_update` -> db fetch before the `playerInCombat` early-exit (`pf_fade.lua:306-307`). Cheap, but unregistering the two health events on `PLAYER_REGEN_DISABLED` and re-registering on `PLAYER_REGEN_ENABLED` would make combat cost exactly zero. Only pursue if profiling points here; profiling is time-intensive and the current post-Optimization-2 guard makes this low priority. [headless-testable if implemented: assert health events unregistered after `PLAYER_REGEN_DISABLED` and restored after `PLAYER_REGEN_ENABLED`.]


## Minor Cleanups
1. [x] `addon.module_defaults.pf` (`pf_defaults.lua:43-44`) is written and never read; only `.st`, `.sv`, and `.audio_volumes` are consumed anywhere. Drop it or add its consumer; the registry key abbreviations are also inconsistent across modules (`pf`/`ob`/`sv`/`st` vs full `audio_volumes`). Resolved 2026-07-07: removed the unused Player Frame defaults registry write; Player Frame continues to own defaults through `M.defaults`.
