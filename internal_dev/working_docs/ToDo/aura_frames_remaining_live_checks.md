...
- Validate Frame BG on Short Buffs, Static / Long Buffs, Timed Buffs, and Debuffs in Bar and Icon modes: local Frame BG should toggle immediately, Aura Shared BG Colors participation should force it visible and apply the shared Frame BG color, and the matching Buff or Debuff All the Colors override should replace the final RGBA independently. Bar Mode must retain its configured-width base and grow by full-width rows; Icon Mode must remain one continuous rectangle following the native visible-icon bounds, without segmented tiles, gaps, overlapping alpha, stale geometry, Lua errors, or blocked actions. Repeat OOC, in combat, and after returning OOC.
