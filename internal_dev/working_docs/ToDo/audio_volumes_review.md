...
- [ ] 4. `read_channel_percent` ignores an active manual Quick Pick. The read gate checks only Fishing/Combat while the matching write path also checks `M._manual_situation_active_key`. Normal controls and copy/seed helpers can therefore read temporary Quick Pick CVars instead of cached normal values. Include the manual-active flag in the read gate.
