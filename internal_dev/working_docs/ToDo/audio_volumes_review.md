...
1. Situations controls retain stale Fishing, Combat, and Quick Pick tables after ARM reset. `BuildSituationsTab` captures `focus_db`, `combat_db`, `quiet_custom_db`, and `entry.db` slider tables, all replaced by reset. Post-reset edits can therefore update only orphaned tables. Fix with live DB access or tab rebuild; add focused triggered/manual situation reset tests.
2. Situations list and cached panels are not rebuilt after ARM reset. Reset deletes custom Quick Picks but the tab-local list and panels remain bound to the old entries. Expose a refresh/rebuild path from `BuildSituationsTab` and invoke it from `on_reset_complete`.
3. `read_channel_percent` ignores an active manual Quick Pick. The read gate checks only Fishing/Combat while the matching write path also checks `M._manual_situation_active_key`. Normal controls and copy/seed helpers can therefore read temporary Quick Pick CVars instead of cached normal values. Include the manual-active flag in the read gate.
4. Normal-panel test-sound preview writes CVars, violating the documented rule that Normal Volumes preview must not write CVars. `play_situation_preview("current", test_sound_key)` has no profile but still creates a cache and delayed restore. Skip the cache/timer when `profile_db` is nil.
5. A situation-preview restore can overwrite a Normal Volume edit made during its two-second window when no temporary situation is active. `set_current_sound_channel_percent` neither cancels nor updates the preview cache, and reads can also observe preview CVars. Verify and route/cancel the pending preview restore as needed.
