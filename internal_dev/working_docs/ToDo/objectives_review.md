...
2. `set_count_setting()` calls `ensure_title_event_frame()` then `update_title_event_registrations()`, which calls ensure again (`ob_section_count.lua:376-377`, `317-318`). Low-risk duplicate setup cleanup.
3. Naming drift: module memory calls the color toggle `Color BG`, the code label is `Custom BG` (`ob_background.lua:1116`). Align the memory wording on its next edit (do not rename the control without a request).
4. Duplicate `get_objective_tracker()` in `ob_position.lua:42-48` and `ob_background.lua:92-98`; identical body. Hoist one copy onto `M` per the one-deterministic-path rule. Moderate because it touches two runtime files and load-order expectations.
5. `set_wow_background_opacity()` double-writes on success (`ob_background.lua:320-345`): after `OnSystemSettingChange` (or `UpdateSystemSettingValue`) succeeds it also calls `ObjectiveTrackerManager:SetOpacity(percent)`. If the belt-and-suspenders write is load-bearing, comment why; otherwise drop it. Moderate because the live/Edit Mode interaction may need in-game confirmation.
