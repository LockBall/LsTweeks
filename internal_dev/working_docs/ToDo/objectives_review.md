...
3. Unreachable border block in `set_background_color_enabled()` (`ob_background.lua:1042-1059`): `is_background_border_enabled()` does not depend on `background_color_enabled`, so `border_was_enabled ~= border_is_enabled` can never be true from this toggle and the offset/sync block inside it is dead. Matches the documented independence of Border from the color toggle, but the code implies a coupling that does not exist.
4. `DEFAULT_BACKGROUND_COLOR` aliases the live defaults table (`ob_background.lua:11` points at `DEFAULTS.objectives.background_color`). Today every use is read-only and `addon.apply_defaults()` fill-copies tables, so the DB never aliases it, but any future write through `DEFAULT_BACKGROUND_COLOR` (or passing it somewhere that mutates its color argument) corrupts module defaults for the session. A local copy at file scope would make it inert.
5. Diagnostics-only falsy swallow in region status (`ob_background.lua:854-857`): `is_desaturated and is_desaturated(region) or nil` and the blend/texture/atlas siblings report `nil` when the real value is `false`, so `bg_region_<n>_desaturated=false` can never appear in `/lst status objectives`. Harmless for control flow but misleading when using the documented region diagnostics to debug tinting.


## Optimization Candidates
1. `sync_objective_border()` re-anchors unconditionally on every background sync (`ob_background.lua:225-252`): `ClearAllPoints` + two `SetPoint` calls plus a Show/Hide on each hooked `tracker:Update` (`ob_background.lua:756-759` queues a sync per update, and both collapsed branches of `sync_objective_background` call it). Cache the last anchor like `background_color_overlay_anchor` does (`ob_background.lua:494-500`) and skip when the anchor and enabled state are unchanged.
2. Per-sync allocation before the signature skip (`ob_background.lua:283-296`, `409-420`, `519`): every `sync_objective_background` builds a fresh color table (`get_background_color`) plus ~6 strings (`get_color_signature`/`get_background_signature`) even when the resulting signature matches and the call early-returns. Setting a dirty flag from the GUI write paths, or comparing raw db fields before building strings, would remove the churn from the hooked tracker-update path.
...
5. `mark_tracker_dirty()` on already-satisfied state (`ob_auto_collapse.lua:110-112`, `128-130`): when the tracker is already collapsed/expanded, apply still forces a Blizzard relayout via `MarkDirty` — twice at login because PLAYER_ENTERING_WORLD applies twice — and the `ObjectiveTrackerManager:UpdateAll()` fallback is a heavy insecure-context call. If the desired state already holds, doing nothing is likely correct; if the dirty-mark exists to kick the background sync hook, `queue_background_sync` is the cheaper direct route.
