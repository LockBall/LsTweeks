...
6. [x] Settings-page chrome constants duplicated across four files. Centralized shared Objectives settings layout in `M.SETTINGS_LAYOUT` from `ob_defaults.lua`; each settings file now references its group slice while preserving the existing page offsets and dimensions. `check_fast.ps1 -Changed` passed; focused in-game settings-page smoke still recommended for visual spacing.
