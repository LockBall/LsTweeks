...
## Latent Traps
1. Secret-value guard gap in the spell fallback: `info.cooldownStartTime or 0` and `info.cooldownDuration or 0` are returned unguarded (`sv_state.lua:94`) while `maxCharges`/`currentCharges` are `issecretvalue`-checked; `M.refresh()` later runs `start_time > 0` and arithmetic on them (`sv_main.lua:505-510`), which would hard-error if Blizzard ever marks those fields secret. Same gap for `UnitPowerDisplayMod()` compared with `> 1` unguarded (`sv_state.lua:46-47`). Memory's Charge State rule is "guard secret values with issecretvalue" — apply it to all four fields.
