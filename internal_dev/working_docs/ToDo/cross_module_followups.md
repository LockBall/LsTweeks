...
- [x] 8. Hot-path cache candidates — audited event buckets, tickers, `OnUpdate`, and render/layout loops. Audio Volumes pre-resolves event playback slots, Player Frame caches its health curve, Objectives coalesces tracker/title work and skips unchanged titles, Aura Frames caches runtime/render data, and Skyriding Vigor caches render context and progress cadence. No additional cache has a sufficiently explicit owner, invalidation path, stale-state fallback, and measured benefit to justify its complexity.
