...
- [x] 13. Priority: Medium | Expected CPU Efficiency Impact: Medium | Change Risk: Medium - Implement a layout/static runtime config cache using frame-local invalidation. Result: `M.update_auras()` resolves scalar/layout config once per frame cache lifetime (`bar_mode`, width, spacing, growth, max icons, tooltip flag, timer text/swipe booleans, sort mode, cooldown overlay state), and `setup_layout()` reuses that resolved cache when available. Existing preset/custom settings updates, resize refresh, profile/reset refresh, and module re-enable refresh invalidate the frame cache. Profiling showed `is_timer_text_enabled` and `uses_cooldown_icon_overlay` no longer appeared in the report, while `get_setting` dropped from about 142.8 calls/sec to 90.7 calls/sec versus the prior run. Colors remain uncached for item 14.
