...
- `tests/test_af_managed.lua`: WoW 12.1 managed AuraContainer foundation and AF12-05.1/05.2 — initialization-before-constraints, synchronized processing/visibility lifecycle gating, startup shell refresh without legacy Aura-update events, combat/OOC shell-alpha transitions, explicit auto-sizing icon/bar flow layout, native icon/name/duration/stack/bar and out-of-combat tooltip bindings, accessible-button filtering and status observability, unavailable-API fallback, and Debuff event/scan isolation.
