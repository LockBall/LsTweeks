...
- `tests/test_af_managed.lua`: managed AuraContainer foundation — initialization-before-constraints, processing/visibility lifecycle gating, public `C_CooldownViewer` record input, accessible-button filtering, status observability, module gating, and unavailable-API fallback.
