...
- `tests/test_ob_auto_collapse.lua`: Objectives visibility-only Auto-Collapse, manual-open persistence, combat deferral, addon-owned visibility restoration, preservation of preexisting hidden/native state, and the prohibition on calling native section or parent collapse.
...
- `tests/test_af_scan_config.lua`: custom-frame cleanup, setting fallback, runtime color resolution, custom secret-timing classification, managed CDM Aura delegation, public-API-only CDM cooldown scans, and the CDM render prohibition on unit-aura ordering reads.
- `tests/test_af_cdm_layout_refresh.lua`: public CDM layout saves install one secure post-hook and queue the settling-aware addon category refresh without replacing the original API call; addon cooldown cells use Blizzard's dark cooldown-swipe color instead of the opaque template default.
...
- `tests/test_af_managed.lua`: managed AuraContainer foundation — initialization-before-constraints, processing/visibility lifecycle gating, effective saved CDM category overrides with public cooldown metadata, globally empty-provider startup fallback, intentionally empty category preservation, accessible-button filtering, status observability, module gating, and unavailable-API fallback.
- `tests/test_af_native_visibility.lua`: reversible BuffFrame/DebuffFrame parent suppression plus viewer-independent CDM alpha hiding, with no Edit Mode forcing or Blizzard viewer script hooks.
- `tests/test_af_managed_debuff_presentation.lua`: managed Debuff native bindings, HARMFUL groups, auto-sizing layout, mode-specific backgrounds, move controls, and combat/OOC aggregate fade behavior.
- `tests/test_af_managed_static_long_presentation.lua`: learned Static / Long `HELPFUL` inclusion groups, cache-revision refresh, color-policy backgrounds, native icon/name/duration/stack/bar bindings, accessible OOC color updates, resize propagation, and Bar/Icon group switching.
- `tests/test_af_managed_short_presentation.lua`: Short Buff managed backend, five-minute native maximum-duration filtering, live threshold refresh, expiration-only ordering, mover tooltip, and absence of legacy icon/event ownership.
...
- `tests/test_af_learned_buffs.lua`: OOC helpful-Aura duration learning, secret/malformed-value rejection, Static / Long inclusion classification, reclassification, combat blocking, and cache clearing.
