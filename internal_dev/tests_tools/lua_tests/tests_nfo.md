...
- `tests/test_af_managed.lua`: WoW 12.1 managed AuraContainer foundation and AF12-05.1 — initialization-before-constraints, synchronized processing/visibility lifecycle gating, startup shell refresh without legacy events, explicit auto-sizing flow layout, native out-of-combat tooltip wiring, accessible-button filtering and status observability, unavailable-API fallback, and the Debuff preset's event/scan isolation plus native `HARMFUL` icon group.
