...
h.test("color picker clears live callbacks after its session closes", function()
    ...
    local presets = ColorPickerFrame._lstweeks_presets
    h.ok(presets and #presets.swatches >= 10, "shared color picker supplies its default preset palette")
    presets.swatches[2]:Click()
    h.eq(db.background_color.r, 1, "Orange preset applies its default red channel")
    h.eq(db.background_color.g, 0.5, "Orange preset applies its default green channel")
    ColorPickerFrame.__opts.cancelFunc()
